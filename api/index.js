...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (url === '/api/work-orders/my' && method === 'GET') {
      const decoded = authenticateToken(req);
      if (!decoded) return res.status(401).json({ message: 'Unauthorized' });
      const query = `
        SELECT wo.id, wo.code, wo.title, wo.description, wo.priority, wo.status,
        ...
        ORDER BY wo.id DESC
      `;
      const wos = await pool.query(query, [decoded.id]);
      return res.status(200).json(wos.rows);
    }

    if (url === '/api/work-orders/customer' && method === 'GET') {
      const decoded = authenticateToken(req);
      if (!decoded) return res.status(401).json({ message: 'Unauthorized' });
      const query = `
        SELECT wo.id, wo.code, wo.title, wo.description, wo.priority, wo.status,
    ...
    if (url === '/api/reports/dashboard' && method === 'GET') {
      ...
      const breachedWos = await pool.query("SELECT COUNT(*) FROM work_orders WHERE sla_due_at IS NOT NULL AND sla_due_at < NOW() AND status NOT IN ('COMPLETED', 'CLOSED', 'CANCELLED')");
      const lowStock = await pool.query("SELECT COUNT(*) FROM parts WHERE stock_qty <= min_stock_level");
    ...
    if (url === '/api/reports/technicians' && method === 'GET') {
    ...
    return res.status(200).json({ message: 'KEYSTONE Vercel Serverless API Running' });
  } catch (err) {
